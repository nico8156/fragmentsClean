...
class CompleteEditorialSourceConsultationTest {
 @Test void stores_signals_before_advancing_checkpoint_and_releases_lease(){
  ...
  assertThat(new CompleteEditorialSourceConsultation(repo,signals,new DeterministicDateTimeProvider()).execute(id,"w",List.of(item),"e","x",null)).isEqualTo(1);
 ...
 @Test void rejects_foreign_signal_without_advancing_checkpoint(){
  ...
  assertThatThrownBy(()->new CompleteEditorialSourceConsultation(repo,new Signals(),new DeterministicDateTimeProvider()).execute(id,"w",List.of(foreign),null,null,null)).hasMessageContaining("mismatch"); assertThat(repo.source.snapshot().leaseOwner()).isEqualTo("w");
 }
