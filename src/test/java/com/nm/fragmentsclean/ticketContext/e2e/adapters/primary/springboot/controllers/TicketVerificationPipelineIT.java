...
public class TicketVerificationPipelineIT extends AbstractTicketBaseE2E {
	...
	void verify_ticket_goes_analyzing_then_completed_and_projection_is_updated() throws Exception {
		...
		// WHEN: dispatch outbox -> kafka (will duplicate accepted to ticket-events +
		// ticket-verification-requested)
		...
		// THEN: projection already has ANALYZING (from TicketVerifyAcceptedEvent on
		// ticket-events)
		await().atMost(Duration.ofSeconds(5)).untilAsserted(() -> {
		...
		// At this point the worker listener should have consumed
		// ticket-verification-requested
		...
		// WHEN: dispatch completed -> kafka (ticket-events) (+ ws via router if
		// enabled)
		outboxEventDispatcher.dispatchPending();
