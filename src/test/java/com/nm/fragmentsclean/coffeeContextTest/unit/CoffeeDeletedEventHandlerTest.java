...
class CoffeeDeletedEventHandlerTest {
	@Test
	void deletes_coffee_projections_then_publishes_projection_updated_sync_event() {
		...
		var handler = new CoffeeDeletedEventHandler(
				summaryRepository,
				photoRepository,
				openingHoursRepository,
				syncPublisher);
		var event = coffeeDeletedEvent();
		...
		assertThat(syncEvent.hints()).containsExactly("deleted", "summary", "photos", "openingHours");
	}

	private CoffeeDeletedEvent coffeeDeletedEvent() {
		return new CoffeeDeletedEvent(
