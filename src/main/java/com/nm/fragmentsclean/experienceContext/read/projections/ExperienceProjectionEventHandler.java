...
public final class ExperienceProjectionEventHandler{
    ...
    public void handle(ExperienceIntegrationEvents.MediaChanged event){projections.apply(event);publish("experiences","coffee",event.coffeeId().toString(),event.version(),event.occurredAt(),"media");if(event.userId()!=null)publish("experiences","user",event.userId().toString(),event.version(),event.occurredAt(),"media");publish("experience-moderation","experience",event.experienceId().toString(),event.version(),event.occurredAt(),"media");}
