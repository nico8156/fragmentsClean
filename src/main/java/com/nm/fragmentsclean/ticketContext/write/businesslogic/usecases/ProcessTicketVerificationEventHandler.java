...
public class ProcessTicketVerificationEventHandler implements EventHandler<TicketVerifyAcceptedEvent> {
    ...
    public void handle(TicketVerifyAcceptedEvent evt) {
        ...
        // Idempotence: si terminal, on ignore (ou tu peux publier un "completed" idempotent)
        ...
        TicketVerificationCompletedEvent completed = switch (result) {
            ...
            case TicketVerificationProvider.Rejected rej -> {
                ...
                yield new TicketVerificationCompletedEvent(
                        ...
                        evt.clientAt(),
                        null,
                        new TicketVerificationCompletedEvent.Rejected(rej.reasonCode(), rej.message()),
                        "openai",
                        rej.providerTraceId()
                );
            }
            ...
            case TicketVerificationProvider.FailedRetryable fail -> {
                // Ici: tu peux laisser le ticket en ANALYZING, ou poser un statut "FAILED" plus tard
                // Pour rester simple: on n'altère pas le ticket, on push juste un completed “FAILED_RETRYABLE”
                yield new TicketVerificationCompletedEvent(
                        UUID.randomUUID(),
                        evt.commandId(),
                        evt.ticketId(),
                        ...
                        evt.clientAt(),
                        null,
                        new TicketVerificationCompletedEvent.Rejected("FAILED_RETRYABLE", fail.message()),
                        "openai",
            ...
            case TicketVerificationProvider.FailedFinal fail -> {
                yield new TicketVerificationCompletedEvent(
                        ...
                        evt.clientAt(),
                        null,
                        new TicketVerificationCompletedEvent.Rejected("FAILED_FINAL", fail.message()),
                        "openai",
                        fail.providerTraceId()
                );
            }
