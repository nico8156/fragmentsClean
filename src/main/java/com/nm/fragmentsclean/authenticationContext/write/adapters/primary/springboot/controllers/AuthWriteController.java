...
import com.nm.fragmentsclean.authenticationContext.write.adapters.primary.springboot.controllers.dto.AppleMobileLoginRequestDto;
...
import com.nm.fragmentsclean.authenticationContext.write.businesslogic.usecases.AppleLoginCommand;
import com.nm.fragmentsclean.authenticationContext.write.businesslogic.usecases.AppleLoginResult;
...
public class AuthWriteController {
  ...
  public StudioGoogleAuthConfig studioConfig() {
    ...
    requirePresent(
        googleOAuthProperties.getStudioRedirectUri(), "google.oauth.studio-redirect-uri");
  ...
  public ResponseEntity<GoogleLoginResponseDto> googleStudio(
      @RequestBody GoogleMobileLoginRequestDto body) {
    ...
    GoogleLoginResult google =
        commandBus.dispatchWithResult(
  ...
  public ResponseEntity<GoogleLoginResponseDto> googleMobile(
      @RequestBody GoogleMobileLoginRequestDto body) {
    requirePresent(body.authorizationCode(), "authorizationCode");
    requirePresent(body.codeVerifier(), "codeVerifier");
    requirePresent(body.redirectUri(), "redirectUri");
    ...
    GoogleLoginCommand command =
        new GoogleLoginCommand(
            body.authorizationCode().trim(), body.codeVerifier().trim(), mobileIosRedirectUri);
  ...
  public ResponseEntity<GoogleLoginResponseDto> appleMobile(
      @RequestBody AppleMobileLoginRequestDto body) {
    requirePresent(body.identityToken(), "identityToken");
    requirePresent(body.authorizationCode(), "authorizationCode");
    AppleLoginResult result =
        commandBus.dispatchWithResult(
            new AppleLoginCommand(
                body.identityToken().trim(), body.authorizationCode().trim(), body.displayName()));
    return ResponseEntity.ok(
        new GoogleLoginResponseDto(
            result.accessToken(),
            result.refreshToken(),
            new GoogleLoginResponseDto.UserSummary(
                result.userId(), result.displayName(), result.email(), result.avatarUrl())));
  }
  ...
  private static GoogleLoginResponseDto toResponse(GoogleLoginResult result) {
    ...
    return new GoogleLoginResponseDto(result.accessToken(), result.refreshToken(), userSummary);
  }

  public record StudioGoogleAuthConfig(
      String clientId, String redirectUri, String authorizationUri, String scope) {}
}
